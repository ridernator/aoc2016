...
std::vector<std::tuple<int64_t, int64_t>> visits;
...
int main() {
    ...
    for (const auto& command : commands) {
        ...
        for (int64_t index = 0; index < amount; ++index) {
            advance(direction, 1, x, y);

            std::tuple<int64_t, int64_t> visit = {x, y};

            if (std::find(visits.begin(), visits.end(), visit) == visits.end()) {
